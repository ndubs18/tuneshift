                ...
                <div className={styles.playlistCardDropdown}>
                    ...
                    {//select the target spotify playlist
                        sourcePlatform === 'Apple Music' ?
                            ...
                            :
                            <button onClick={async () => {
                                window.location.replace(`${process.env.AUTH_SERVICE_BASE_URL}/login/Apple?source=${sourcePlatform}&sourcePlaylistId=${playlistId}&sourcePlaylistName=${name}&target=Apple Music`)
                            }}>Transfer</button>
                    }
                </div>
            </>
